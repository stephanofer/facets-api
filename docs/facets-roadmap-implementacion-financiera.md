...
| ----- | ------------------------------------------ | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| 0     | **Checkpoint: alinear contratos actuales** | nada                       | Antes de sumar módulos nuevos, conviene asegurarse de que schema, seeds y código ya implementado estén contando la misma historia. Si no, las próximas features nacen sobre contratos inconsistentes. | `schema.prisma`, seeds, auth/workspaces/subscriptions/storage                                                                       | Si hoy avatar o preferencias usan nombres/campos distintos a los del schema actual, cerralo primero. |
| 1     | **Kernel financiero compartido**           | auth + workspace ya listos | Todas las features financieras van a necesitar las mismas reglas: scoping por workspace, periodos, money math, invalidación de cache y semántica analítica común.                                     | `Workspace`, `WorkspaceSettings`, reglas sobre `financialDataUpdatedAt`, helpers/shared services                                    | Cada write financiero debe tocar `financialDataUpdatedAt` y dejar listo el recálculo afectado.       |
| 2     | **Catálogos y setup financiero base**      | 1                          | Antes de crear cuentas y movimientos necesitás una base estable de moneda, país, locale financiero y defaults de arranque.                                                                            | `Currency`, `Country`, resolución de `WorkspaceSettings`                                                                            | Un workspace nuevo arranca con `baseCurrency = PEN` y `monthStartDay = 26`.                          |
| 3     | **Accounts core**                          | 1, 2                       | Todo el dominio financiero cuelga de cuentas. Sin esto no existe ni saldo, ni cashflow, ni deuda real.                                                                                                | `Account`, `LoanProfile`, `CreditCardProfile`, `DebtProfile`, `LentMoneyProfile`                                                    | Crear `Visa Interbank` como `CREDIT_CARD` con saldo inicial 800.                                     |
